...
public class RoomFactory : MonoBehaviour
{
    ...
    public GameObject CreateRoom(Enums.RoomTypes roomType, Enums.RoomSizes roomSize)
    {
        RoomDefData DefData = GetRoomDefData(roomType, roomSize);
    ...
    public RoomDefData GetRoomDefData(Enums.RoomTypes roomType, Enums.RoomSizes roomSize)
    {
        foreach (RoomDefData DefData in Constants.RoomDefinitions)
        {
            if ((DefData.RoomType == roomType) && (DefData.RoomSize == roomSize))
            {
                return (DefData);
            }
        }
